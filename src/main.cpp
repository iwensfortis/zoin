...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        if (pfrom->cleanSubVer != "/Satoshi:0.8.7.5/" && pfrom->cleanSubVer != "/Zoin Core:0.9.0.0/" && pfrom->cleanSubVer != "/Zoin Core:0.9.0/")
        {
    ...
    int64 nFees = 0;
    {
        ...
        for (map<uint256, CTransaction>::iterator mi = mempool.mapTx.begin(); mi != mempool.mapTx.end(); ++mi)
        {
            ...
            if (tx.IsZerocoinSpend())
            {
                ...
                //printf("nBlockSize = %llu\n", nBlockSize);
                //printf("nTxSize = %d\n", nTxSize);
                //printf("nBlockSize + nTxSize  = %llu\n", nBlockSize + nTxSize );
                //printf("######################################\n\n\n\n\n");
